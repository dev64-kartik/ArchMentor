...
**M3 dogfood findings (2026-04-25 / 2026-04-26, carry into M4):**
...
- **A pending brain call eats the previous call's `speak` reply on TTL** (observed 2026-04-26, both directions). Symmetric: the dropped reply can be either origin. Examples from one session: (a) candidate asks a question while drawing → `TURN_END speak` queued, next pending `CANVAS_CHANGE` dispatch takes 4 s, reply drops at age 12 s; (b) candidate adds a database to the canvas → `CANVAS_CHANGE speak` queued, candidate then speaks → next `TURN_END` dispatch takes 7 s, canvas reply drops at age 11 s. In both cases the candidate experiences silence (or worse, hears the *other* event's reply). Three viable levers in M4: (a) **prioritise queue drain over the next dispatch** — whenever the queue holds a fresh "speak" payload, drain it before starting the next brain call regardless of pending event type; (b) **freshness-aware TTL** — extend TTL on any payload while a brain call is in-flight (so it survives one competing dispatch), regardless of origin; (c) the **call-count throttle from the first bullet** would also reduce competing canvas dispatches. Lands alongside streaming LLM→TTS — both touch the dispatcher's serialisation gate. Regression tests: (i) `TURN_END speak` + 12 s `CANVAS_CHANGE` dispatch chain → speak delivered, not dropped; (ii) `CANVAS_CHANGE speak` + 12 s `TURN_END` dispatch → canvas speak delivered, not dropped.
