...
- **Speak-while-drawing causes the brain's reply to TTL-drop silently** (observed 2026-04-26). When the candidate asks a question while editing the canvas, the brain decides `speak` (~8 s) but the reply queues *behind* the next pending `CANVAS_CHANGE` brain call (~4 s more). By the time the queue drainer gets an idle window, the reply is past its 10 s TTL and `queue.dropped_stale` fires with no audio. The candidate hears silence and re-asks, which produces a second (paid) brain call that just barely beats the TTL. Three viable levers in M4: (a) **prioritise queue drain over canvas-only dispatches** — when the head of `pending` is `CANVAS_CHANGE` and the queue holds a fresh "speak" payload, drain first then dispatch; (b) **content-aware TTL** — extend TTL when the utterance was generated in response to a `TURN_END` (so it survives one canvas dispatch) but keep it short for `CANVAS_CHANGE`-only replies; (c) the **call-count throttle from the first bullet** would also reduce competing canvas dispatches. Lands alongside streaming LLM→TTS — both touch the dispatcher's serialisation gate. Regression test: simulate `TURN_END speak` followed by a 12 s canvas-only dispatch chain and assert the speak payload is delivered, not dropped.
