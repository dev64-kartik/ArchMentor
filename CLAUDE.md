...
M0 (foundation) landed 2026-04-19. M1 (voice loop skeleton) ✅ done 2026-04-21. M2 (brain MVP + session persistence) ✅ landed 2026-04-22 on `feat/m2-brain-mvp` — Opus tool-use brain, serialized event router with coalescer, utterance queue + speech-check gate, Redis `SessionState` with no-TTL + CAS, `brain_snapshots` ingest route + replay CLI, Hinglish-friendly STT. See `docs/plans/2026-04-22-001-feat-m2-brain-mvp-plan.md` for the execution checkpoint and deferred work. **Next:** M3 — real session lifecycle (`POST /sessions`, `POST /sessions/{id}/end`, `DELETE /sessions/{id}` with full cascade, `/session/new` UI), Excalidraw canvas embed + `canvas-scene` LiveKit topic (full-scene transport, no diffs), agent-side canvas parser with structural label fencing for the brain prompt, `canvas_change` priority on `RouterEvent` so canvas preempts `turn_end` in the coalescer, `canvas_snapshots` write-through, candidate-UX bundle (thinking-elapsed copy, mic-health dot, keepalive Fetch on tab close, synthetic recovery utterance, opening calibration line), plus M2 carry-overs (body-size ASGI middleware on `/events`+`/snapshots`, snapshot-ingest TOCTOU fix, retry-chain budget on Anthropic client). **Streaming LLM→TTS is explicitly M4**, paired with Kokoro sentence-chunked synthesis — do not pull it into M3. See `docs/plans/2026-04-25-001-feat-m3-canvas-and-session-lifecycle-plan.md` and `docs/brainstorms/2026-04-25-m3-plan-refinements-requirements.md` (the 2026-04-25 refinements pass that scoped down diffs/debouncer and added the candidate-UX bundle).
