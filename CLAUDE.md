...
- **Synthetic recovery utterances (R27) are ledger-discriminated.** When the brain times out and the speech-check gate allows it, the agent says *"Let me come back to that — please continue."* and writes a `utterance_ai` ledger event with `synthetic: true` + `reason: "brain_timeout"` (or `reason: "anthropic_api_connection_during_wait_for"` when the SDK surfaces the cancellation as `APIConnectionError` post-deadline). M5/M6 replay can filter these out. Capped at one per session via `_apology_used` on the router.
